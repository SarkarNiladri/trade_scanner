...
const CFG = {
  ...
  ATR_PERIOD: 14, DAILY_ATR_PERIOD: 14,
...
export async function getNiftyTrend() {
  try {
    ...
    if (df.length < 55) return { trend: 'neutral', adx: 0 };
...
async function getStockDailyData(symbol) {
  try {
    ...
    if (df.length < 55) return { trend: 'neutral', atr: 0, rsi: 50, emaBull: false };
    const closes = df.map(x => x.close);
    const highs  = df.map(x => x.high);
    const lows   = df.map(x => x.low);
    const e20 = ema(closes, 20);
...
export async function analyzeStock(symbol, niftyTrend) {
  try {
    ...
    if (df.length < CFG.EMA_LONG + 20) return null;
    ...
    if (macdH > 0 && prevM <= 0)      { buyScore  += 2; if (emaBullish) reasons.push('MACD ↑ cross'); }
    else if (macdH < 0 && prevM >= 0) { sellScore += 2; if (emaBearish) reasons.push('MACD ↓ cross'); }
