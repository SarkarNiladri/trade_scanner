...
const TTL = 3 * 3600;   // 3 hours (was 30 min)
...
async function classifyAll(headlinesBySector) {
  ...
  const prompt =
    ...
    `For each section below, decide BULLISH, BEARISH, or NEUTRAL, assign a 0-100 confidence, and give a one-line reason (max 10 words).\n\n` +
    `${sections}\n\n` +
  ...
  try {
    ...
    // Normalize and attach headlines
    const out = {};
    for (const name of Object.keys(headlinesBySector)) {
